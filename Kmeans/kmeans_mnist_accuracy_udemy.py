...
####################################################################
# My Notes:
# This script shows two ways of evaluating how good the
# clusters found by k-means are.
#
# Purity value can be used just when you have labels.
# DBI can be used without labels which is what we need in an
...
# This version only takes into account the cluster assigned (the higher responsability)
def purity2(Y, R):
...
# This version takes into account the matrix of responsabilities
def purity(Y, R):
    # maximum purity is 1, higher is better
    N, K = R.shape
    p = 0
